...
const (
	...
	Proprietary = "01"
	// International Standard Book Number, pre-2007, unhyphenated (10 characters) – now
	// DEPRECATED in ONIX for Books, except where providing historical information for compatibility
	// with legacy systems. It should only be used in relation to products published before 2007 – when
	// ISBN-13 superseded it – and should never be used as the ONLY identifier (it should always be
	// accompanied by the correct GTIN-13 / ISBN-13).
	ISBN_10 = "02"
	// GS1 Global Trade Item Number, formerly known as EAN article number (13 digits).
	GTIN_13 = "03"
	// UPC product number (12 digits).
	UPC = "04"
	// International Standard Music Number (M plus nine digits). Pre-2008 – now DEPRECATED in ONIX for
	// Books, except where providing historical information for compatibility with legacy systems. It
	// should only be used in relation to products published before 2008 – when ISMN-13 superseded it
	// – and should never be used as the ONLY identifier (it should always be accompanied by the
	// correct ISMN-13).
	ISMN_10 = "05"
	...
	// and/or 15 should always be the ISBN that is used for ordering from the supplier
	...
	JP_e_code = "27"
	// Unique number assigned by the Chinese Online Library Cataloging Center (see
